...
describe("shareRows", () => {
  it("keeps rank, picker, ticker, last, overall, and today P/L", () => {
    const rows = shareRows([
      {
        ...
        name: "AXS SPAC",
        last: 26.85,
        overall: { dollars: 22.55, percent: 0.0563 },
        today: { dollars: 23.11, percent: 0.0578 },
      },
    ]);
...
describe("portfolioTodayDollars", () => {
  it("sums today dollars across lots", () => {
    assert.equal(
      portfolioTodayDollars([
        { today: { dollars: 1.5 } },
        { today: { dollars: -0.25 } },
...
describe("formatShareWhen", () => {
  it("names the Eastern clock time", () => {
    assert.equal(
      formatShareWhen(Date.parse("2026-09-14T14:35:00.000Z")),
      "Sep 14, 10:35 AM"
    );
  });
});

describe("shareCaptureStyle", () => {
  it("pins the clone on-canvas so off-screen position is not copied", () => {
    const style = shareCaptureStyle();
