...
function sessionOffsets(points) {
  if (!points.length) return [];
  const offsets = [0];
  const dts = [];
  for (let i = 1; i < points.length; i += 1) {
    const dt = points[i].t - points[i - 1].t;
    if (dt > 0 && dt <= SESSION_GAP_MS) dts.push(dt);
  }
  const typical = dts.length
    ? dts.slice().sort((a, b) => a - b)[Math.floor(dts.length / 2)]
    : 5 * 60 * 1000;
  let x = 0;
  for (let i = 1; i < points.length; i += 1) {
    const dt = Math.max(0, points[i].t - points[i - 1].t);
    x += dt > SESSION_GAP_MS ? typical : dt;
    offsets.push(x);
  }
  return offsets;
}
...
function formatPortfolioDelta(value, pnl, unit = "$") {
  if (!Number.isFinite(value) || !Number.isFinite(pnl)) return "—";
  const cost = value - pnl;
  const percent = cost !== 0 ? pnl / cost : 0;
  const delta = unit === "%" ? formatPercent(percent) : formatSignedPlain(pnl);
  return `${value.toFixed(2)} (${delta})`;
}
...
function shareRows(ranked) {
  return (ranked || []).map((row) => ({
    rank: row.rank,
    picker: row.picker,
    ticker: row.ticker,
    dollars: row.overall.dollars,
    percent: row.overall.percent,
    todayDollars: row.today.dollars,
    todayPercent: row.today.percent,
  }));
}

function portfolioTodayDollars(rows) {
  return (rows || []).reduce((sum, row) => sum + (row.today?.dollars || 0), 0);
}

function formatShareWhen(ms) {
